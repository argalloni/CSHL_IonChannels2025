...
class MultiLevelEventDetector_test:
    """
    Interactive event detection for single-channel recordings with multiple conductance levels.
    Automatically identifies baseline as the level closest to zero, with other levels above or below.
    """
    ...
    def __init__(self, traces, time_array, sampling_freq):
        ...
        # Initialize level parameters
        self.baseline_level = None
        self.current_levels = []  # List of current levels (can be positive or negative relative to baseline)
        self.level_names = []     # Names for each level (e.g., 'L1', 'L2', etc.)
        self.detection_thresholds = []  # Midpoint thresholds between levels
    ...
    def auto_detect_levels(self, n_levels=None, method='histogram', plot_result=True):
        """
        Automatically detect current levels from the data
        
        Parameters:
        -----------
        n_levels : int, optional
            Number of levels to detect (including baseline). If None, auto-detect
        method : str
            'histogram' or 'kmeans' for level detection
        plot_result : bool
            Whether to plot the detected levels
            
        Returns:
        --------
        detected_levels : list
            List of detected current levels sorted by proximity to zero
        """
        from scipy import stats
        from sklearn.cluster import KMeans
        
        # Flatten all traces for analysis
        all_data = self.traces.flatten()
        
        if method == 'histogram':
            # Use histogram peaks to identify levels
            hist, bins = np.histogram(all_data, bins=100, density=True)
            ...
            if len(peaks) == 0:
                raise ValueError("Could not detect peaks in histogram")
            
            # Get peak positions (current levels)
            detected_levels = bin_centers[peaks]
            
        elif method == 'kmeans':
            # Use K-means clustering to identify levels
            if n_levels is None:
                # Try different numbers of clusters and use elbow method or silhouette
                from sklearn.metrics import silhouette_score
                silhouette_scores = []
                k_range = range(2, min(8, len(all_data)//1000))  # Reasonable range
                
                for k in k_range:
                    kmeans = KMeans(n_clusters=k, random_state=42, n_init=10)
                    labels = kmeans.fit_predict(all_data.reshape(-1, 1))
                    score = silhouette_score(all_data.reshape(-1, 1), labels)
                    silhouette_scores.append(score)
                
                # Choose k with highest silhouette score
                best_k = k_range[np.argmax(silhouette_scores)]
                print(f"Auto-detected {best_k} levels using silhouette analysis")
            else:
                best_k = n_levels
            ...
            detected_levels = kmeans.cluster_centers_.flatten()
        
        # Sort levels by proximity to zero (baseline is closest to zero)
        detected_levels = sorted(detected_levels, key=abs)
        
        # Set baseline as the level closest to zero
        self.baseline_level = detected_levels[0]
        
        # Set other levels (removing baseline from the list)
        other_levels = detected_levels[1:]
        
        if plot_result:
            self.plot_level_detection(all_data, detected_levels)
        
        print(f"Auto-detected levels (sorted by proximity to zero):")
        print(f"  Baseline (closest to 0): {self.baseline_level:.2f} pA")
        for i, level in enumerate(other_levels):
            print(f"  Level {i+1}: {level:.2f} pA ({'positive' if level > self.baseline_level else 'negative'} relative to baseline)")
        
        # Automatically set these levels
        if other_levels:
            self.set_current_levels(other_levels)
        
        return detected_levels
    
    def plot_level_detection(self, all_data, detected_levels):
        """Plot histogram with detected levels marked"""
        fig, ax = plt.subplots(figsize=(10, 6))
        
        # Plot histogram
        ax.hist(all_data, bins=100, alpha=0.7, density=True, edgecolor='white')
        
        # Mark detected levels
        colors = ['red'] + [f'C{i}' for i in range(len(detected_levels)-1)]
        for i, (level, color) in enumerate(zip(detected_levels, colors)):
            label = 'Baseline' if i == 0 else f'Level {i}'
            ax.axvline(level, color=color, linestyle='--', linewidth=2, 
                      label=f'{label}: {level:.2f} pA')
        
        ax.set_xlabel('Current (pA)')
        ax.set_ylabel('Density')
        ax.set_title('Automatic Level Detection')
        ax.legend()
        ax.grid(True, alpha=0.3)
    ...
    def estimate_baseline(self, method='closest_to_zero'):
        """
        Estimate baseline level from the data
        ...
        -----------
        method : str
            'closest_to_zero', 'percentile', 'mode', or 'minimum'
        """
        if method == 'closest_to_zero':
            # Find the most common current level that's closest to zero
            hist, bins = np.histogram(self.traces.flatten(), bins=100)
            bin_centers = (bins[:-1] + bins[1:]) / 2
            
            # Find the bin center closest to zero with significant occupancy
            min_occupancy = 0.05 * np.max(hist)  # At least 5% of max occupancy
            valid_bins = hist >= min_occupancy
            valid_centers = bin_centers[valid_bins]
            
            if len(valid_centers) > 0:
                self.baseline_level = valid_centers[np.argmin(np.abs(valid_centers))]
            else:
                # Fallback to absolute closest to zero
                self.baseline_level = bin_centers[np.argmin(np.abs(bin_centers))]
                
        elif method == 'percentile':
            # Use the level that's most common (mode of distribution)
            hist, bins = np.histogram(self.traces.flatten(), bins=100)
            self.baseline_level = bins[np.argmax(hist)]
    ...
    def set_current_levels(self, levels, level_names=None):
        """
        Manually set current levels (can be positive or negative relative to baseline)
        
        Parameters:
        -----------
        levels : list
            List of current values (in pA) for each conductance level
            Can be positive or negative relative to baseline
        level_names : list, optional
            Names for each level (default: 'L1', 'L2', etc.)
        """
        if len(levels) > 5:
        ...
        # Sort levels by their distance from baseline, then by their actual value
        # This ensures consistent ordering regardless of positive/negative levels
        levels_with_distance = [(level, abs(level - self.baseline_level)) for level in levels]
        levels_with_distance.sort(key=lambda x: (x[1], x[0]))  # Sort by distance, then by value
        self.current_levels = [level for level, _ in levels_with_distance]
        ...
        # Calculate detection thresholds (midpoints between levels)
        # Create a sorted list of all levels including baseline
        all_levels = [self.baseline_level] + self.current_levels
        all_levels.sort()  # Sort by actual current value
        ...
        print(f"  Baseline: {self.baseline_level:.2f} pA")
        for i, (level, name) in enumerate(zip(self.current_levels, self.level_names)):
            direction = "above" if level > self.baseline_level else "below"
            print(f"  {name}: {level:.2f} pA ({direction} baseline)")
        
        print(f"Detection thresholds: {[f'{t:.2f}' for t in self.detection_thresholds]} pA")

    def get_level_index_from_current(self, current_value):
        """
        Determine which level index a current value belongs to
        ...
        --------
        level_idx : int
            0 for baseline, 1+ for other levels
        """
        # Create sorted list of all levels
        all_levels = [self.baseline_level] + self.current_levels
        all_levels_sorted = sorted(all_levels)
        
        # Find which level this current value is closest to
        closest_level = min(all_levels, key=lambda x: abs(x - current_value))
        
        # Return the index (0 for baseline, 1+ for others)
        if closest_level == self.baseline_level:
            return 0
        else:
            return self.current_levels.index(closest_level) + 1

    def detect_events_single_trace(self, trace_idx, plot_result=False):
        """
        Detect events in a single trace using the specified levels
        Works with both positive and negative levels relative to baseline
        """
        trace = self.traces[trace_idx, :]
        idealized = np.full_like(trace, self.baseline_level)
        events = []
        ...
        # Create sorted list of all levels for threshold calculation
        all_levels = [self.baseline_level] + self.current_levels
        all_levels_sorted = sorted(all_levels)
        
        # Create thresholds between adjacent levels
        thresholds = []
        for i in range(len(all_levels_sorted) - 1):
            threshold = (all_levels_sorted[i] + all_levels_sorted[i+1]) / 2
            thresholds.append(threshold)
        
        # State tracking
        current_level = 0  # Start assuming baseline
        event_start = 0
        
        # State machine for event detection
        for i, current_value in enumerate(trace):
            # Determine which level the current value belongs to
            new_level_idx = self.get_level_index_from_current(current_value)
            
            # Apply hysteresis to prevent rapid switching
            if new_level_idx != current_level:
                # Check if this level change is stable over next few samples
                stable_samples = max(3, int(0.1 * self.sampling_freq / 1000))
                
                if i + stable_samples < len(trace):
                    future_values = trace[i:i+stable_samples]
                    stable = all(self.get_level_index_from_current(val) == new_level_idx 
                               for val in future_values)
                    
                    if not stable:
                        new_level_idx = current_level  # Keep current level if not stable
            ...
            if new_level_idx != current_level:
                ...
                if (i - event_start) >= min_samples:
                    ...
                    # Get amplitude for this level
                    if level_idx == 0:
                        amplitude = self.baseline_level
                    else:
                        amplitude = self.current_levels[level_idx - 1]
    ...
    def validate_levels(self, trace_idx=0):
        ...
        for level in all_levels_sorted:
            if level == self.baseline_level:
                print(f"  Baseline: {level:.2f} pA")
            else:
                level_idx = self.current_levels.index(level) + 1
                direction = "above" if level > self.baseline_level else "below"
                print(f"  Level {level_idx}: {level:.2f} pA ({direction} baseline)")
        ...
        for i in range(len(all_levels_sorted) - 1):
            threshold = (all_levels_sorted[i] + all_levels_sorted[i+1]) / 2
            thresholds.append(threshold)
            print(f"  {all_levels_sorted[i]:.1f} ↔ {all_levels_sorted[i+1]:.1f}: {threshold:.2f} pA")
        
        # Check level occupancy
        print("\nLevel occupancy:")
        for level in all_levels:
            nearby_points = np.sum(np.abs(trace - level) < 2.0)  # Within 2 pA
            level_name = "Baseline" if level == self.baseline_level else f"Level {self.current_levels.index(level) + 1}"
            print(f"  {level_name} ({level:.1f} pA): {nearby_points} points ({nearby_points/len(trace)*100:.1f}%)")
    ...
    def plot_single_trace_result(self, trace_idx, events, idealized):
        ...
        # Plot level lines
        ax.axhline(self.baseline_level, color='k', linestyle='--', alpha=0.7, label='Baseline')
        for i, (level, name) in enumerate(zip(self.current_levels, self.level_names)):
            ax.axhline(level, color=f'C{i+2}', linestyle='--', alpha=0.7, label=name)
        
        # Plot detection thresholds
        all_levels_sorted = sorted([self.baseline_level] + self.current_levels)
        for i in range(len(all_levels_sorted) - 1):
            threshold = (all_levels_sorted[i] + all_levels_sorted[i+1]) / 2
    ...
    def plot_detection_summary(self, duration_hist_bins='auto'):
        ...
        # Plot 2: Event duration histogram for first non-baseline level
        durations = self.calculate_event_durations(level_filter=1)
        first_level_name = list(durations.keys())[0] if durations else 'L1'
        all_durations = durations.get(first_level_name, [])
        ...
        if all_durations:
            ...
            ax.set_xlabel('Event Duration (ms)')
            ax.set_ylabel('Count')
            ax.set_title(f'Event Durations: {first_level_name} (n={len(all_durations)})')
            ax.grid(True, alpha=0.3)
    ...
    def calculate_open_probability(self):
        """
        Calculate open probability considering baseline as closed state
        """
        if not self.event_lists:
            raise ValueError("No events detected. Run detect_all_events() first.")
        ...
        # Initialize time tracking for each level
        for i, name in enumerate(self.level_names):
            level_times[name] = 0
        level_times['Baseline'] = 0
        
        # Calculate times for each level
        for events in self.event_lists:
            for event in events:
                start_time, end_time, level_idx, amplitude = event
                ...
                if level_idx == 0:  # Baseline (closed)
                    level_times['Baseline'] += duration
                else:  # Open states
                    level_name = self.level_names[level_idx - 1]
                    level_times[level_name] += duration
                    open_time += duration
        
        # Calculate probabilities
        po_overall = open_time / total_time if total_time > 0 else 0
        
        po_by_level = {}
        for level_name, time in level_times.items():
            po_by_level[level_name] = time / total_time if total_time > 0 else 0
        
        statistics = {
            'total_time_s': total_time,
            'open_time_s': open_time,
            'closed_time_s': level_times['Baseline'],
